...
from bayesian_inference import calculate_likelihood, calculate_likelihood_manual, compute_bme, bal_selection_criteria,MCMC_MH_post ,compute_post_bme
...
# Function to perform MCMC_MH_post
def perform_MCMC_MH_post_subset(prior_bal, mean, variance, al_old, proir_bal_pdf, mc_size_AL, obs, total_error):
    al_exploration_list = []
    covariance_matrices_list = []

    for i in range(1000):
        al_exploration = MCMC_MH_post(al_old[i], mc_size_AL, obs, variance[i], mean[i], total_error)
        covariance_matrices = np.cov(al_exploration.T)
        al_exploration_list.append(al_exploration)
        covariance_matrices_list.append(covariance_matrices)

    return al_exploration_list, covariance_matrices_list
...
d_size_AL = 1_000#1_000        # number of active learning sets (sets I take from the prior to do the active learning).
mc_size_AL = 50#1_000      # sample size for output space
# Note: d_size_AL+ iteration_limit < mc_size
al_strategy = "RE"   # BAL criteria (Here we want to test different methods)
# BME 6.047643817000919 h
# RE  6.970741360518668 h
# IE  6.287303209900856 h  300_3.5412548239363564 h
...
for Ntp in range(0, iteration_limit+1):
    ...
    if Ntp < iteration_limit:
        ...
        al_exploration = np.zeros((mc_size_AL, n_obs))
        ...
        for iAL in range(0, len(al_unique_index)):
            prior_bal = np.random.normal(size=(mc_size, n_obs)) * surrogate_std[:, al_unique_index[iAL]] + \
                              surrogate_prediction[:, al_unique_index[iAL]]
            
            mean = surrogate_prediction[:, al_unique_index[iAL]]
            
            # # Calculate the variance of the product
            variance = (surrogate_std[:, al_unique_index[iAL]]**2)
            
            proir_bal_pdf=stats.multivariate_normal.pdf(prior_bal, mean=mean,cov = variance) 
            
            al_old = stats.multivariate_normal(mean = surrogate_prediction[:, al_unique_index[iAL]], 
                                    cov = variance,
                                    ).rvs()
            
            prior_bal_list.append(prior_bal)
            mean_list.append(mean)
            variance_list.append(variance)
            al_old_list.append(al_old)
            proir_bal_pdf_list.append(proir_bal_pdf)
            
            
    #         n_cand_groups = 4
    #         a10 = np.array_split(prior_bal_list, n_cand_groups, axis=0)
    #         a11 = np.array_split(mean_list, n_cand_groups, axis=0)
    #         a12 = np.array_split(variance_list, n_cand_groups, axis=0)
    #         a13 = np.array_split(al_old_list, n_cand_groups, axis=0)
    #         a14 = np.array_split(proir_bal_pdf_list, n_cand_groups, axis=0)
    #         # subsets = list(zip( a11, a12, a13))
            
            
 
            
    #         # Run the MCMC_MH_post function in parallel for each element of al_unique_index
    #     results = Parallel(n_jobs=-1)(
    # delayed(MCMC_MH_post)(a11[i], a12[i], a13[i], mc_size_AL, obs, total_error)
    #     for i in range(0,4)
    # )
    ...
    #         covariance_matrices_list.append(covariance_matrices)
        ...
        # for exploration, covariance in results:
        #     al_exploration_list.extend(exploration)
        #     covariance_matrices_list.extend(covariance)
                        
                    
        #     al_exploration_list.append(al_exploration)
            
        #     covariance_matrices = np.cov(al_exploration_list[i].T)
        ...
        for i in range(0, 1000):
            al_exploration = MCMC_MH_post(al_old_list[i],
                            mc_size_AL,
                            obs, 
                            variance_list[i],
                            mean_list[i],
        ...
        for i in range(0, 1000):
        # for i in range(0, 1000):
            al_IE[i], al_BME[i], al_RE[i] = compute_post_bme(covariance_matrices = covariance_matrices_list[i],
                                                                    model_predictions=al_exploration_list[i],
                                                                    observations=obs,
                                                                    var=total_error,
                                                                    prior_pdf=proir_bal_pdf_list[i],
                                                                    # likelihood = llk,
