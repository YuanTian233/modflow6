...
def compute_post_bme(
    ...
    :return: float, float
        BME and RE values
    """
    IE = 0.5 * (np.log(2 * math.pi * math.exp(1)**6) * np.linalg.det(covariance_matrices))
    ...
    # else:
        # likelihood = stats.multivariate_normal.pdf(model_predictions, cov=cov_mat, mean=observations[0, :])
    BME = np.exp(np.mean(np.log(likelihood)) + np.mean(np.log(prior_pdf)) + IE)       
    RE = -np.mean(np.log(prior_pdf))- IE
