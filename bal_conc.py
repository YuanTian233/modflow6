...
# Create a new folder for saving resluts
...
model_name = "50"  # Name of the groundwater model for modflow6
n_zones = 6   # Number of zones for groundwater model according to the zones distrubution
...
from gwatermodel import add_noise
...
tp_i = 10 # number of initial collocation points
...
from gwatermodel import hkfields
from gwatermodel import runmodelflow
...
for Ntp in range(0, iteration_limit+1):
    ...
    BME[Ntp], RE[Ntp], IE[Ntp] = compute_bme(surrogate_prediction.T, conc_obs, total_error,
                                              prior_pdf=prior_pdf_set, prior_samples=prior)
    ...
    if Ntp < iteration_limit:
        ...
        for iAL in range(0, len(al_unique_index)):
            ...
            al_BME[iAL], al_RE[iAL] = compute_bme(model_predictions=al_exploration, observations=conc_obs,
                                                  var=total_error)
